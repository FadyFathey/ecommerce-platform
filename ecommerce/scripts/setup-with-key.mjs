/**
 * Setup script that accepts service role key as command line argument
 * Usage: node scripts/setup-with-key.mjs YOUR_SERVICE_ROLE_KEY
 */
...
// Get service role key from command line argument or environment
const supabaseUrl = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL
let supabaseServiceRoleKey = process.argv[2] || process.env.SUPABASE_SERVICE_ROLE_KEY
...
if (!supabaseServiceRoleKey) {
  ...
  console.error('\nUsage: node scripts/setup-with-key.mjs YOUR_SERVICE_ROLE_KEY')
  console.error('   Or add SUPABASE_SERVICE_ROLE_KEY to .env file')
  process.exit(1)
}
...
async function setupPolicies() {
  ...
  for (const policy of policies) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`   ⚠️  Policy '${policy.name}' - API method not available`)
        console.log(`   ℹ️  Will need manual setup via SQL Editor`)
      }
    } catch (error) {
  ...
  if (successCount === policies.length) {
    ...
    console.log(`\n   ℹ️  Policies need to be set up manually via SQL Editor`)
...
async function main() {
  try {
    ...
    if (error.message.includes('JWT')) {
      console.error('\n💡 The service role key might be incorrect. Please verify:')
      console.error('   1. Go to Supabase Dashboard → Settings → API')
      console.error('   2. Copy the "service_role" key (the secret one)')
      console.error('   3. Make sure you copied the entire key')
    }
